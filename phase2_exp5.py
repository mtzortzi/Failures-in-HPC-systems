...
def main():
    ...
    print('............read phase2............')
    print('....I had already reorder phase2.csv by descending order....')
    phase2_new = pd.read_csv('/various/mtzortzi/from_nikela/ASCENDING_ORDER_METHOD/DESCENDING_ORDER_trains/phase2.csv') 
    ...
    n_sequences2 = phase1_exp4.series_to_supervised(values2, 10, 1)
    ...
    n_features2 = 2 #wids and DT
    n_obs2 = 10 * n_features2 #columns as input for the obs of all features across the previous 10 columns (history size)
    ...
    embedding_matrix2 = np.zeros((len(phase2_new['WIDS'].unique()) + 1, embed_size))
    word_embeddings2 = word_embeddings.T.to_dict('list')
    embedding_list = []
    word_list = []
    for word, i in word2id_all.items():
        if phase2_new['MULTIWORDS'].str.contains(word).any():
    ...
    model = load_model('/various/mtzortzi/from_nikela/ASCENDING_ORDER_METHOD/DESCENDING_ORDER_trains/phase1_exp5/models/model.3696-0.6572.hdf5', custom_objects=dependencies)
    ...
    # Split the data
    train_X_new, X_valid, train_Y_new, Y_valid = train_test_split(train_X, train_Y, test_size=0.28, shuffle= True)
    print('DONE')
    model2, history2 = lstm_model2(train_X_new, X_valid, train_Y_new, Y_valid, batch_size, rnn_size, train_X, train_Y, model, history, vocab_size_all, word_embeddings_phase2, embed_size, initial_epochs)
    print('DONE')
...
def lstm_model2(train_X_new, X_valid, train_Y_new, Y_valid, batch_size, rnn_size, train_X, train_Y, model, history, vocab_size_all, word_embeddings_phase2, embed_size, initial_epochs):
    ...
    model2.add(Embedding(input_dim=len(word_embeddings_phase2),
                    ...
                    trainable=False))
    model2.add(LSTM(rnn_size, dropout=0.5, recurrent_dropout=0.5, return_sequences=True))
    #model2.add(LSTM(rnn_size, return_sequences=True))#, input_shape=(train_X.shape[1], train_X.shape[2])))       
    #model2.add(Dropout(rate=0.2, training=True))
    model2.add(LSTM(rnn_size, dropout=0.5, recurrent_dropout=0.5, return_sequences=True))
    #model2.add(LSTM(rnn_size, kernel_regularizer=l2(1E-2), recurrent_regularizer=l2(1E-2), bias_regularizer=l2(1E-2), return_sequences=True))
    #model2.add(Dropout(rate=0.2, training=True))
    model2.add(Dense(vocab_size_all, activation='elu', name='dense_2')) 
    model2.add(Lambda(lambda x: x[:,-1:,:]))
    learning_rate = 0.00001 * (float(batch_size) / 16) #learning rate 0.01 * (float(batch_size) / 16) 
    optimizer = RMSprop(lr=learning_rate, rho=0.9, decay=1E-2/total_epochs)

    model.load_weights('/various/mtzortzi/from_nikela/ASCENDING_ORDER_METHOD/DESCENDING_ORDER_trains/phase1_exp5/models/model.3696-0.6572.hdf5')
    ...
    ex = '/various/mtzortzi/from_nikela/ASCENDING_ORDER_METHOD/DESCENDING_ORDER_trains/phase2_exp5/models'
    model_dir = os.path.join('/various/mtzortzi/from_nikela/ASCENDING_ORDER_METHOD/DESCENDING_ORDER_trains/phase2_exp5', ex)
    print(model_dir)
    if not os.path.isdir(model_dir):
        os.makedirs(model_dir)

    ex1 = '/various/mtzortzi/from_nikela/ASCENDING_ORDER_METHOD/DESCENDING_ORDER_trains/phase2_exp5_tensorboard'
    ...
    #callbacks
    csv_logger = CSVLogger('/various/mtzortzi/from_nikela/ASCENDING_ORDER_METHOD/DESCENDING_ORDER_trains/phase2_exp5/training.log', separator=',', append=False)
    ...
    for i, w in enumerate(class_weights2):
        if unique_classes2[i] ==5: 
            listd.append((unique_classes2[i], w/4000))
        elif unique_classes2[i] ==9:
            listd.append((unique_classes2[i], 100*w))
        #elif unique_classes2[i] ==10: 
        #    listd.append((unique_classes2[i], 10*w))
        # elif unique_classes2[i] ==11: 
            # listd.append((unique_classes2[i], w/10))
        elif unique_classes2[i] ==14: 
            listd.append((unique_classes2[i], 150*w))
        elif unique_classes2[i] ==16:
            listd.append((unique_classes2[i], 40*w))
        elif unique_classes2[i] ==20:
            listd.append((unique_classes2[i], 10*w))
        elif unique_classes2[i] ==22: 
            listd.append((unique_classes2[i], 300*w))
        elif unique_classes2[i] ==24: 
            listd.append((unique_classes2[i], 6000*w))
        elif unique_classes2[i] ==26:
            listd.append((unique_classes2[i], 40*w))
        elif unique_classes2[i] ==28:
            listd.append((unique_classes2[i], 2000*w))
        elif unique_classes2[i] ==29:
            listd.append((unique_classes2[i], 155*w))
        elif unique_classes2[i] ==31:
            listd.append((unique_classes2[i], 150*w))
        elif unique_classes2[i] ==33: 
            listd.append((unique_classes2[i], 300*w))
        elif unique_classes2[i] ==34: 
            listd.append((unique_classes2[i], 100*w))
        elif unique_classes2[i] ==38: 
            listd.append((unique_classes2[i], 100*w))
        elif unique_classes2[i] ==39: 
            listd.append((unique_classes2[i], 1000*w))
        elif unique_classes2[i] ==40: 
            listd.append((unique_classes2[i], 100*w))
        elif unique_classes2[i] ==41:
            listd.append((unique_classes2[i], 100*w))
        elif unique_classes2[i] ==43: 
            listd.append((unique_classes2[i], 5000*w))
        elif unique_classes2[i] ==46: 
            listd.append((unique_classes2[i], 100*w))
        elif unique_classes2[i] ==47:
            listd.append((unique_classes2[i], 10000*w))
        elif unique_classes2[i] ==49:
            listd.append((unique_classes2[i], 300*w))
        elif unique_classes2[i] ==50: 
            listd.append((unique_classes2[i], 4000*w))
        elif unique_classes2[i] ==51: 
            listd.append((unique_classes2[i], 200*w))
        elif unique_classes2[i] ==53: 
            listd.append((unique_classes2[i], 500*w))
        elif unique_classes2[i] ==54: 
            listd.append((unique_classes2[i], 5000*w))
        elif unique_classes2[i] ==55: 
            listd.append((unique_classes2[i], 5000*w))
        elif unique_classes2[i] ==57:
            listd.append((unique_classes2[i], 200*w))
        elif unique_classes2[i] ==59:
            listd.append((unique_classes2[i], 1000*w))
        elif unique_classes2[i] ==60:
            listd.append((unique_classes2[i], 10000*w))
        elif unique_classes2[i] ==61:
            listd.append((unique_classes2[i], 60000*w))
        elif unique_classes2[i] ==62:
            listd.append((unique_classes2[i], 30000*w))
        elif unique_classes2[i] ==65:
            listd.append((unique_classes2[i], 50000*w))
        elif unique_classes2[i] ==66:
            listd.append((unique_classes2[i], 12000*w))
        elif unique_classes2[i] ==67:
            listd.append((unique_classes2[i], 30000*w))
        elif unique_classes2[i] ==69:
            listd.append((unique_classes2[i], 20000*w))
        elif unique_classes2[i] ==70: 
            listd.append((unique_classes2[i], 200000*w))
        elif unique_classes2[i] ==71: 
            listd.append((unique_classes2[i], 60000*w))
        elif unique_classes2[i] ==72: 
            listd.append((unique_classes2[i], 100000*w))
        elif unique_classes2[i] ==73: 
            listd.append((unique_classes2[i], 40000*w))
        elif unique_classes2[i] ==76:
            listd.append((unique_classes2[i], 400000*w))
        elif unique_classes2[i] ==80:
            listd.append((unique_classes2[i], 60000*w))
        elif unique_classes2[i] ==94: 
            listd.append((unique_classes2[i], 8000000*w))
        else:
    ...
    print('Training')
    start = time.time()
    history2 = model2.fit(train_X_new, train_Y_new, class_weight=weights2,
    ...
    model2.save('/various/mtzortzi/from_nikela/ASCENDING_ORDER_METHOD/DESCENDING_ORDER_trains/phase2_exp5/multi_model.h5') 
