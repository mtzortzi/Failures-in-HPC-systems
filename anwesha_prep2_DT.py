...
def phase2_prep(filename):
    ...
    indexNames = phase1_labeled[phase1_labeled['AN_LABELS'] == "SAFE"].index
    ...
    for key, msg in phase1_nodewise_UE.groupby(['LOCATION']):
        #print("the group for Location node '{}' has {} rows".format(key,len(msg))) 
        if msg['AN_LABELS'].str.contains('ERROR').any() or msg['AN_LABELS'].str.contains('INDICATOR_ERROR').any():
            list5.append(msg)
    print('---------------------------------------------------------------------')
    ...
    for key, msg in phase1_nodewise_UE.groupby(['LOCATION']):
        #print("the group for Location node '{}' has {} rows".format(key,len(msg))) 
        if msg['AN_LABELS'].str.contains('INDICATOR_ERROR').any():
            list6.append(msg)
        else:
            continue
    ...
    if filename == '/various/mtzortzi/from_nikela/phase1_labeled.csv':
        phase.to_csv('/various/mtzortzi/from_nikela/phase1_nodewise_UE_fc.csv', index=False, sep=',', quoting=csv.QUOTE_ALL)
        print('DONE WITH phase1_nodewise_UE_fc which contains only nodes with failure chains')
    elif filename == '/various/mtzortzi/from_nikela/anwesha_labeled.csv':
        phase.to_csv('/various/mtzortzi/from_nikela/anwesha_pre2.csv', index=False, sep=',', quoting=csv.QUOTE_ALL)
...
def DT_calculation(filename):
    ...
    for idx, msg in phase_df.iterrows():
        if msg.AN_LABELS == 'INDICATOR_ERROR':
    ...
    print(set(phase_df['AN_multiwords']))
    freq_dist_pos = FreqDist(phase_df['AN_multiwords'])
    print(freq_dist_pos.most_common(5))
    print(len(phase_df['AN_multiwords']))

    if filename == '/various/mtzortzi/from_nikela/phase1_nodewise_UE_fc.csv':
        phase_df.to_csv('/various/mtzortzi/from_nikela/phase2.csv', index=False, sep=',', quoting=csv.QUOTE_ALL)
    elif filename == '/various/mtzortzi/from_nikela/test70_ndw_labeled.csv':
        phase_df.to_csv('/various/mtzortzi/from_nikela/phase3.csv', index=False, sep=',', quoting=csv.QUOTE_ALL)
    elif filename == '/various/mtzortzi/from_nikela/anwesha_pre2.csv':
        phase_df.to_csv('/various/mtzortzi/from_nikela/phase2_anwesha.csv', index=False, sep=',', quoting=csv.QUOTE_ALL)        
